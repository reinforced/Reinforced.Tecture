...
namespace Reinforced.Tecture.Commands
{
    ...
    public class TectureCommandRunException : Exception
    {
        ...
        internal TectureCommandRunException(CommandBase command, Exception innerException) : base($"An error occured during execution of {(string.IsNullOrEmpty(command.Annotation)?$"command {command.GetType().Name}":$"'{command.Annotation}'")}. See inner exception for details.", innerException)
        {
            Command = command;
        }
