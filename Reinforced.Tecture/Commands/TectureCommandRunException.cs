...
namespace Reinforced.Tecture.Commands
{
    ...
    public class TectureCommandRunException : Exception
    {
        ...
        internal TectureCommandRunException(CommandBase command, Exception innerException) : base($"An error occured during run of command {command.GetType().Name}. See inner exception for details.", innerException)
        {
            Command = command;
        }
